...
def get_signal_model(settings: dict, df: DataFrame) -> MRBeRT:
    ...
    """
    # setting colnames
    # if other types of outcomes can be modeled in the future
    if settings["cov_model"]["outcome_type"] == 'binary':
        col_obs="ln_rr"
        col_obs_se='ln_rr_se'
    elif settings["cov_model"]["outcome_type"] == 'continuous':
        col_obs="md"
        col_obs_se='md_se'
    data = MRData()
    data.load_df(
...
def get_cov_finder_linear_model(df: DataFrame) -> MRBRT:
    ...
    col_obs = "ln_rr" if "ln_rr_se" in df.columns else "md"
    col_obs_se = "ln_rr_se" if "ln_rr_se" in df.columns else "md_se"
...
def get_linear_model_summary(
    ...
    outcome_type = "binary" if "ln_rr_se" in df.columns else "continuous"
    ...
    residual = df.ln_rr.values[index] - df.signal.values[index] * beta_info[0] if outcome_type == 'binary' else df.md.values[index] - df.signal.values[index] * beta_info[0]
    residual_sd = np.sqrt(
        df.ln_rr_se.values[index] ** 2 + df.re_signal.values[index] ** 2 * gamma_info[0] if outcome_type == 'binary' else df.md_se.values[index] ** 2 + df.re_signal.values[index] ** 2 * gamma_info[0]
    )
...
def plot_signal_model(
    outcome_type: str, name: str, summary: dict, df: DataFrame, signal_model: MRBeRT
) -> Figure:
    ...
    # plot data
    _plot_data(outcome_type, name, summary, df, ax, signal_model=signal_model)
...
def plot_linear_model(
    ...
    # plot data
    _plot_data(outcome_type, name, summary, df, ax[0], signal_model, linear_model)
...
def _plot_data(
    ...
    # use ref_y to substitute ref_ln_rr
    ref_y = signal_model.predict(
        MRData(
            covs={
    ...
    if linear_model is not None:
        ref_y *= linear_model.beta_soln[0]
    # use alt_y to substitute alt_ln_rr
    alt_y = ref_y + df.ln_rr.values if outcome_type == "binary" else ref_y + df.md.values

    # shift data position normalize to tmrel
    if summary["normalize_to_tmrel"]:
        risk = np.linspace(*summary["risk_bounds"], 100)
        signal = get_signal(signal_model, risk)
        alt_y -= signal.min()  # use alt_y to substitute alt_ln_rr

    # plot data points
    index = df.is_outlier == 1
    ax.scatter(
        alt_risk,
        alt_y,  # use alt_y to substitute alt_ln_rr
        s=5 / df.ln_rr_se.values if outcome_type == "binary" else 5 / df.md_se.values,
        color="#008080",
        alpha=0.5,
        edgecolor="none",
    )
    ax.scatter(
        alt_risk[index],
        alt_y[index],  # use alt_y to substitute alt_ln_rr
        s=5 / df.ln_rr_se.values[index] if outcome_type == "binary" else 5 / df.md_se.values[index],
        color="red",
        alpha=0.5,
        marker="x",
    )
    for x_0, y_0, x_1, y_1 in zip(alt_risk, alt_y, ref_risk, ref_y):  # use alt_y to substitute alt_ln_rr and ref_y to substitute ref_ln_rr
    ...
    ylabel = "ln relative risl" if outcome_type == "binary" else "mean difference"
...
def _plot_funnel(
    ...
    residual = df.ln_rr.values - df.signal.values * beta[0] if outcome_type == "binary" else df.md.values - df.signal.values * beta[0]
    residual_sd = np.sqrt(df.ln_rr_se.values**2 + df.re_signal.values**2 * gamma[0]) if outcome_type == "binary" else np.sqrt(df.md_se.values**2 + df.re_signal.values**2 * gamma[0])
